...
const checkAccountBalance = async (
  ...
  console.log(
    `Balance of ${address} on ${nodeName}: ${formatEther(balance)} ETH`
  );
  return balance;
};
...
const sendTransaction = async (
  ...
  console.log(
    `Attempting to send ${amount} ETH from ${signer.address} to ${toAddress} on ${nodeName}...`
  );
  ...
  console.log(
    `Transaction details: nonce=${nonce}, gasPrice=${suggestedGasPrice.toString()}, gasLimit=21000`
  );
  ...
  try {
    ...
    console.log(
      `Transaction successful on ${nodeName}. Tx Hash: ${response.hash}`
    );
    if (receipt) {
      console.log(
        `Block number: ${receipt.blockNumber}, Gas used: ${receipt.gasUsed}`
      );
    }
    return response;
  } catch (error) {
...
(async () => {
  try {
    ...
    const minernodeAddress = fs.readFileSync(
      path.join(blockchainDataPath, "signer", "keys", "address"),
      { encoding: "utf-8" }
    );
    const minernodePrivateKey = fs.readFileSync(
      path.join(blockchainDataPath, "signer", "keys", "key.priv"),
      { encoding: "utf-8" }
    );
    ...
    const blockNumberChecker = await getBlockNumber(
      rpcNodeProviders[0],
      "Block number reference"
    );

    for (const [index, rpcProvider] of rpcNodeProviders.entries()) {
      const rpcBlockNumber = await getBlockNumber(
        rpcProvider,
        `RPC Node ${RPC_PORT_NODE_LIST[index]}`
      );
      console.log(
        `➡️ RPC Node ${RPC_PORT_NODE_LIST[index]} block number: ${rpcBlockNumber}`
      );

      if (
        rpcBlockNumber === blockNumberChecker ||
        rpcBlockNumber + 1 === blockNumberChecker
      ) {
        console.log(
          `✅ RPC Node ${RPC_PORT_NODE_LIST[index]} is synchronized.`
        );
      } else {
        console.warn(
          `⚠️ RPC Node ${RPC_PORT_NODE_LIST[index]} is NOT synchronized.`
        );
      }
    }
    ...
    if (rpcNodeProviders.length > 0) {
      ...
      const initialMinerBalance = await checkAccountBalance(
        rpcNodeProviders[0],
        minernodeSigner.address,
        "Signer Node (Pre-Tx)"
      );
      const initialRecipientBalance = await checkAccountBalance(
        rpcNodeProviders[0],
        recipientAddress,
        "Recipient (Pre-Tx)"
      );
      ...
      const finalMinerBalance = await checkAccountBalance(
        rpcNodeProviders[0],
        minernodeSigner.address,
        "Signer Node (Post-Tx)"
      );
      const finalRecipientBalance = await checkAccountBalance(
        rpcNodeProviders[0],
        recipientAddress,
        "Recipient (Post-Tx)"
      );
      ...
      if (
        finalMinerBalance < initialMinerBalance &&
        finalRecipientBalance > initialRecipientBalance
      ) {
        console.log("✅ Transaction successful: Balances updated as expected.");
      } else {
        console.error(
          "❌ Transaction failed: Balances did NOT update as expected."
        );
      }
    } else {
      console.warn(
        " ❌Cannot perform transaction test: No RPC nodes available."
      );
    }
